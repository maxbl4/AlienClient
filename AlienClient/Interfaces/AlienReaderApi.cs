...
namespace AlienClient.Interfaces
{
    ...
    {
        public abstract Task<string> SendRecieve(string command);
        ...
        private async Task<T> Property<T>(object value = null, bool checkPrefix = true, [CallerMemberName]string name = null)
        {
            ...
            if (string.IsNullOrEmpty(stringValue))
                response = await SendRecieve($"{name}?");
            else
                response = await SendRecieve($"{name} = {stringValue}");
        ...
        {
            return AlienValueConverter.ToStrongType<T>(await SendRecieve(name));
        }
